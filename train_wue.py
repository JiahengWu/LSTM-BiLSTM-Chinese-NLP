...
def compute_accuracy(v_xs,v_ys):
    global pred
    prediction=sess.run(pred,feed_dict={xs:v_xs,ys:v_ys})
...
def LSTM_Model(xs,embedding_size,text_len,n_hidden_units,train_text):
    ...
    W_out = tf.Variable(tf.constant(0.1, shape=[n_hidden_units, text_len]))
    b_out = tf.Variable(tf.constant(0.1, shape=[text_len]))

    result = tf.matmul(outputs[-1], W_out) + b_out
    return result
...
pred=LSTM_Model(xs,embedding_size,text_len,n_hidden_units,train_text)
cost=tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.cast(ys,tf.float32),logits=pred))
accuracy=tf.reduce_mean(tf.cast(tf.equal(tf.argmax(pred,1),tf.argmax(ys,1)),tf.float32))
