...
class LSTM():
    def __init__(self,xs,vocabdict,embedding_size,train_text,n_hidden_units,batch_size,text_len,keep_prob1,keep_prob2):
        ...
        x_in = tf.reshape(x_in, [-1, embedding_size])
        ...
        x_out = tf.transpose(outputs, [1, 0, 2])
        text_mask = tf.reduce_mean(x_out, axis=0)
        W_m = tf.Variable(tf.random_normal([n_hidden_units, n_hidden_units], mean=0, stddev=0.1))
        b_m = tf.Variable(tf.constant(0.1, shape=[n_hidden_units]))
        masks = tf.nn.sigmoid(tf.matmul(text_mask, W_m) + b_m)
        x_out = tf.multiply(x_out, masks)
        x_out = tf.transpose(x_out, [1, 0, 2])
        x_lstm_out = tf.reshape(x_out, [-1, n_hidden_units])
