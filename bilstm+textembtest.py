...
modelpath='/home/lisi/wjh/result/bilstm+textemb/'
...
class LSTM():
    def __init__(self,xs,vocabdict,embedding_size,train_text,n_hidden_units,batch_size,text_len,
        ...
        with tf.variable_scope('text_emb'):
            ...
            outputs_text, states_text = tf.nn.bidirectional_dynamic_rnn(lstm_cell_fw_text, lstm_cell_bw_text, x_in_text, dtype=tf.float32,time_major=False)
            output_text=tf.concat(outputs_text,2)#[batch,step,n_hidden_unites]
            output_text=tf.nn.dropout(output_text,keep_prob2)

        with tf.variable_scope('final'):
            x_in_all=tf.concat([x_in,output_text],axis=2)
            dim=embedding_size+2*n_hidden_units_text
