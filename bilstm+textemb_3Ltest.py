...
modelpath='/home/lisi/wjh/result/bilstm+textemb_3L/'
...
def compute_accuracy(v_xs,v_ys):
    global result
    prediction=sess.run(result,feed_dict={xs:v_xs,ys:v_ys,keep_prob1:1,keep_prob2:1,keep_prob3:1,keep_prob4:1})
...
def testresult_perbatch(v_xs,v_ys):
    global result
    test_result=sess.run(tf.sigmoid(result),feed_dict={xs:v_xs,ys:v_ys,keep_prob1:1,keep_prob2:1,keep_prob3:1,keep_prob4:1})
    return test_result


def testresult_whole(istest=True):
    if istest:
        ckpt = tf.train.get_checkpoint_state(modelpath + 'bestmodel/')
        if ckpt:
            print('havebestmodel')
    ...
    while n<=total_n_test:
        t_xs,t_ys,n,tn=load_test_data(batch_size,test_text,test_label,vocabdict,n)
        if tn is not None:
            test_total_result.extend(testresult_perbatch(t_xs,t_ys))
        else:
            test_total_result.extend(testresult_perbatch(t_xs,t_ys)[:total_n_test-n])
            break
    return test_total_result

def load_test_len_sen():
    test_len_sen=[]
    f=codecs.open(testdir,'r',encoding='utf-8')
    data=f.readlines()
    for i in range(len(data)):
        words=data[i].split(' ')
        test_len_sen.append(len(words))
    return test_len_sen

def total_test_process():
    test_total_result=testresult_whole()
    print(test_total_result[0])
    test_len_sen=load_test_len_sen()
    test_result_copy1=copy.deepcopy(test_total_result)
    test_result_copy2 = copy.deepcopy(test_total_result)
    test_result_copy3 = copy.deepcopy(test_total_result)
    MRRr=MRR(test_result_copy1,test_len_sen,test_label)
    HIT2r=HIT2(test_result_copy2,test_len_sen,test_label)
    HITKr=HITK(test_result_copy3,test_len_sen,test_label)
    print('MRR: %5f'%MRRr)
    print('HIT2: %5f' % HIT2r)
    print('HITK0.2: %5f' % HITKr)

def MRR(test_total_result,test_len_sen,test_label):
    MRR_result=[]
    for i in range(len(test_len_sen)):
        k=1
        truth=test_label[i]
        pred=test_total_result[i]
        truthid = np.argmax(truth)
        while True:
            test_argmax_id=np.argmax(pred)
            if test_argmax_id==truthid:
                MRR_result.append(float(1.0/k))
                break
            else:
                pred[test_argmax_id]=-10
                k=k+1
    MRRr=sum(MRR_result)/len(MRR_result)
    return MRRr

def HIT2(test_total_result,test_len_sen,test_label):
    HIT2_result = []
    for i in range(len(test_len_sen)):
        truth = test_label[i]
        pred = test_total_result[i]
        ...
        for k in range(2):
            test_argmax_id = np.argmax(pred)
            if test_argmax_id == truthid:
                HIT2_result.append(1.0)
                break
            else:
                pred[test_argmax_id] = -10
                if k==1:
                    HIT2_result.append(0.0)
    HIT2r = sum(HIT2_result) / len(HIT2_result)
    return HIT2r

def HITK(test_total_result,test_len_sen,test_label,hitk=0.2):
    HITK_result = []
    for i in range(len(test_len_sen)):
        per_sen_k = int(test_len_sen[i]*hitk)
        truth = test_label[i]
        pred = test_total_result[i]
        truthid = np.argmax(truth)
        for k in range(per_sen_k):
            test_argmax_id = np.argmax(pred)
            if test_argmax_id == truthid:
                HITK_result.append(1.0)
                break
            else:
                pred[test_argmax_id] = -10
                if k==per_sen_k-1:
                    HITK_result.append(0.0)
    HITKr = sum(HITK_result) / len(HITK_result)
    return HITKr

class LSTM():
    def __init__(self,xs,vocabdict,embedding_size,train_text,n_hidden_units,batch_size,text_len,
                 keep_prob1, keep_prob2, keep_prob3, keep_prob4, n_hidden_units_text):
        embeddings = load_word_embeddings(vocabdict,embedding_size,train_text)
        x_in = tf.nn.embedding_lookup(embeddings, xs)  # [batch,text_len]-->[batch,text_len,emb_size]
        x_in_word = tf.reshape(x_in, [-1, embedding_size])
        ...
        x_in_word = tf.matmul(x_in_word, W) + b
        x_in_word = tf.reshape(x_in_word, [-1, text_len, n_hidden_units])
        with tf.variable_scope('word'):
            lstm_cell_fw_word = tf.nn.rnn_cell.BasicLSTMCell(n_hidden_units)
            lstm_cell_bw_word = tf.nn.rnn_cell.BasicLSTMCell(n_hidden_units)
            outputs_word, states_text = tf.nn.bidirectional_dynamic_rnn(lstm_cell_fw_word, lstm_cell_bw_word, x_in_word,
                                                                        dtype=tf.float32, time_major=False)
            output_word = tf.concat(outputs_word, 2)  # [batch,step,n_hidden_unites]
            output_word = tf.nn.dropout(output_word, keep_prob1)

        with tf.variable_scope('text_emb'):
            x_in_word = tf.reshape(x_in, [-1, embedding_size])
            W = tf.Variable(tf.truncated_normal([embedding_size, n_hidden_units_text], stddev=0.1))
            b = tf.Variable(tf.constant(0.1, shape=[n_hidden_units_text]))
            x_in_text = tf.matmul(x_in_word, W) + b
            x_in_text = tf.reshape(x_in_text, [-1, text_len, n_hidden_units_text])
            lstm_cell_fw_text = tf.nn.rnn_cell.BasicLSTMCell(n_hidden_units_text)
            lstm_cell_bw_text = tf.nn.rnn_cell.BasicLSTMCell(n_hidden_units_text)
            outputs_text, states_text = tf.nn.bidirectional_dynamic_rnn(lstm_cell_fw_text, lstm_cell_bw_text, x_in_text,
                                                                        dtype=tf.float32, time_major=False)
            output_text = tf.concat(outputs_text, 2)  # [batch,step,n_hidden_unites]
            output_text = tf.nn.dropout(output_text, keep_prob2)

        with tf.variable_scope('final'):
            x_in_all = tf.concat([x_in, output_word, output_text], axis=2)
            dim = 2 * n_hidden_units + embedding_size + 2 * n_hidden_units_text
            lstm_cell_fw_all = tf.nn.rnn_cell.BasicLSTMCell(dim)
            lstm_cell_bw_all = tf.nn.rnn_cell.BasicLSTMCell(dim)
            outputs_all, states_all = tf.nn.bidirectional_dynamic_rnn(lstm_cell_fw_all, lstm_cell_bw_all, x_in_all,
                                                                      dtype=tf.float32)
            output_all = tf.concat(outputs_all, 2)
            output_all = tf.nn.dropout(output_all, keep_prob3)
        x_lstm_out = tf.reshape(output_all, [-1, 2 * dim])
        print(x_lstm_out.get_shape())
        W_lstm_out = tf.Variable(tf.constant(0.1, shape=[2 * dim, 1]))
        b_lstm_out = tf.Variable(tf.constant(0.1, shape=[1]))
        result = tf.matmul(x_lstm_out, W_lstm_out) + b_lstm_out
        result = tf.nn.dropout(result, keep_prob4)
...
def train(batch_size,train_text,train_label,vocabdict):
    ...
    for i in range(10000):
        ...
        train_result = sess.run([trainstep, accuracy],
                                feed_dict={xs: train_x, ys: train_y, keep_prob1: 0.5, keep_prob2: 0.5,
                                           keep_prob3: 0.5, keep_prob4: 0.5})
        strtrain = 'train accuracy: %5f' % (train_result[1])
        print(strtrain)
