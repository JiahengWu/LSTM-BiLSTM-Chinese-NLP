...
modelpath='/home/lisi/wjh/result/bilstm+dropout/'
...
def compute_accuracy(v_xs,v_ys):
    global result
    prediction=sess.run(result,feed_dict={xs:v_xs,ys:v_ys,keep_prob1:1,keep_prob2:1})
...
def testresult_perbatch(v_xs,v_ys):
    global result
    test_result=sess.run(tf.sigmoid(result),feed_dict={xs:v_xs,ys:v_ys,keep_prob1:1,keep_prob2:1})
    return test_result
    ...
    for i in range(len(test_len_sen)):
        ...
        while True:
            ...
            if test_argmax_id==truthid:
                ...
                if test_argmax_id>test_len_sen[i]:
...
class LSTM():
    def __init__(self,xs,vocabdict,embedding_size,train_text,n_hidden_units,batch_size,text_len,keep_prob1,keep_prob2):
        ...
        x_in = tf.nn.dropout(x_in, keep_prob=keep_prob1)
        ...
        result = tf.nn.dropout(result, keep_prob2)
...
def train(batch_size,train_text,train_label,vocabdict):
    ...
    for i in range(10000):
        ...
        sess.run(trainstep,feed_dict={xs: train_x,ys: train_y,keep_prob1:0.25,keep_prob2:0.5})
        stracc=sess.run(accuracy,feed_dict={xs:train_x,ys:train_y,keep_prob1:1,keep_prob2:1})
...
model=LSTM(xs,vocabdict,embedding_size,train_text,n_hidden_units,batch_size,text_len,keep_prob1,keep_prob2)
result=model.result
